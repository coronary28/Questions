/* PATCH V5 CLEAN: safe compatibility patch */
(function(){
  'use strict';
  ...
  if(!state.historyDeleteState){
    state.historyDeleteState = { baseScope:'all', ids:[] };
  }
  ...
  function ensureGroupOrder(groups, sectionType, subjectName){
    ...
    fallbackIds.forEach(id => {
      if(ids.includes(id) && !clean.includes(id)) clean.push(id);
    });
    ids.forEach(id => {
  ...
  function moveGroupToBottomByInfo(subjectName, sectionType, groupId){
    ...
    const arr = Array.isArray(state.groupPreferences[key])
      ? state.groupPreferences[key].slice()
      : getOriginalOrderIds(subjectName, sectionType);
  ...
  function addProgressIdsForQuestion(q){
    if(!q || typeof addProgressId !== 'function') return;
  ...
  function removeProgressIdsFromKey(key, ids){
    if(!state.progress || !state.progress[key]) return;
  ...
  function rerenderAfterChecklistChange(){
    try{ if(typeof saveChecklistStore === 'function') saveChecklistStore(); }catch(e){}
    try{ if(typeof saveProgressStore === 'function') saveProgressStore(); }catch(e){}
    try{ if(typeof renderChecklist === 'function') renderChecklist(); }catch(e){}
    try{
      if(typeof renderChecklistSubject === 'function' && el('checklist-subject-screen') && el('checklist-subject-screen').classList.contains('active')){
        renderChecklistSubject();
      }
    }catch(e){}
    try{ if(typeof renderSubjects === 'function' && state.browseMode === 'all') renderSubjects(); }catch(e){}
    try{ if(typeof updateStatisticsIfOpen === 'function') updateStatisticsIfOpen(); }catch(e){}
    try{ if(typeof renderMemories === 'function') renderMemories(); }catch(e){}
    try{
      if(el('selection-screen') && el('selection-screen').classList.contains('active') && typeof renderSelectionScreenWithEnhancements === 'function'){
  ...
  function ensureGlobalHomeButtons(){
    document.querySelectorAll('.screen').forEach(screen => {
      ...
      /* مهم: لا يظهر زر Home داخل الامتحان */
  ...
  .selection-group-item.dragging{opacity:.55}
  .selection-group-item.drag-over{border-color:var(--primary);background:var(--primary-soft)}
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if(state.statsExclusions && state.statsExclusions.excludedSections){
      state.statsExclusions.excludedSections.years = false;
    }
