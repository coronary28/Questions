const state = {
  ...
  memoryOffset: { weekly: 0, monthly: 0, yearly: 0 },
  timerWarningPlayed: false,
  groupPreferences: {}
};

const STORAGE_KEYS = {
  settings:'medical-app-settings-v12',
  progress:'medical-app-progress-v12',
  favorites:'medical-app-favorites-v12',
  wrong:'medical-app-wrong-v12',
  examState:'medical-app-exam-state-v12',
  subjectPrefs:'medical-app-subject-prefs-v12',
  questionLog:'medical-app-question-log-v12',
  examHistory:'medical-app-exam-history-v12',
  firstVisit:'medical-app-first-visit-v12',
  statsExclusions:'medical-app-stats-exclusions-v2',
  subjectStatsSettings:'medical-app-subject-stats-settings-v2',
  checklist:'medical-app-checklist-v1',
  groupPrefs:'medical-app-group-prefs-v2'
};
...
const BACKGROUND_SOUNDS = {
  none:{file:''},
  cafeteria:{file:'Cafeteria.mp3'},
  'after-exam':{file:'After the exam.mp3'},
  beach:{file:'Beach.mp3'},
  forest:{file:'Forest.mp3'},
  fireplace:{file:'Fireplace.mp3'},
  'rain-thunder':{file:'Rain-thunder.mp3'},
  'rain-window':{file:'Rain-window.mp3'}
};
const LETTERS = 'ABCDE';
...
function getSubjectColor(subjectName){
  const darkPalette=['#93c5fd','#86efac','#fcd34d','#c4b5fd','#fda4af','#67e8f9','#fdba74','#f9a8d4'];
  const lightPalette=['#1d4ed8','#15803d','#b45309','#7c3aed','#be123c','#0f766e','#9a3412','#9d174d'];
  const palette=isDarkTheme()?darkPalette:lightPalette;
  const subjects=sortSubjects(state.subjects).map(s=>s.name);
  const idx=Math.max(0, subjects.indexOf(subjectName)) % palette.length;
  return palette[idx];
}
function formatHistorySubLabel(item){
  if(!item || !item.groups || !item.groups.length) return item.sourceLabel || 'عام';
  return item.groups.map(g=>g.type==='ai' ? `${g.name} (AI)` : g.name).join('، ');
}
...
function getStatsSectionPalette(type){
  ...
  const map = dark
    ? { lecture:{accent:'#f8fafc', bg:'rgba(255,255,255,0.10)'}, year:{accent:'#fde68a', bg:'rgba(253,230,138,0.12)'}, ai:{accent:'#86efac', bg:'rgba(134,239,172,0.12)'} }
    : { lecture:{accent:'#1d4ed8', bg:'rgba(29,78,216,0.08)'}, year:{accent:'#92400e', bg:'rgba(245,158,11,0.10)'}, ai:{accent:'#047857', bg:'rgba(16,185,129,0.10)'} };
  return map[type] || (dark ? {accent:'#f8fafc', bg:'rgba(255,255,255,0.10)'} : {accent:'#1d4ed8', bg:'rgba(29,78,216,0.08)'});
}
...
function saveMemoryStores(){
  localStorage.setItem(STORAGE_KEYS.questionLog, JSON.stringify(state.questionsFirstSeen));
  localStorage.setItem(STORAGE_KEYS.examHistory, JSON.stringify(state.examHistory));
  if(state.firstVisit) localStorage.setItem(STORAGE_KEYS.firstVisit, String(state.firstVisit));
}
function loadMemoryStores(){
  state.questionsFirstSeen = loadJSON(STORAGE_KEYS.questionLog, {});
  state.examHistory = loadJSON(STORAGE_KEYS.examHistory, []);
  const fv = localStorage.getItem(STORAGE_KEYS.firstVisit);
  state.firstVisit = fv ? Number(fv) : Date.now();
  if(!fv) localStorage.setItem(STORAGE_KEYS.firstVisit, String(state.firstVisit));
}
...
function addProgressId(key, qid){
  if(!state.progress[key]) state.progress[key]={questionIds:[]};
  if(!state.progress[key].questionIds.includes(qid)) state.progress[key].questionIds.push(qid);
}
function loadGroupPreferences(){ state.groupPreferences = loadJSON(STORAGE_KEYS.groupPrefs, {}); }
function saveGroupPreferences(){ localStorage.setItem(STORAGE_KEYS.groupPrefs, JSON.stringify(state.groupPreferences || {})); }

function normalizeSubjectPreferences(){
  const ids=state.subjects.map(s=>s.id);
  state.subjectPreferences.order=(state.subjectPreferences.order||[]).filter(id=>ids.includes(id));
  state.subjectPreferences.pinned=(state.subjectPreferences.pinned||[]).filter(id=>ids.includes(id));
  ids.forEach(id=>{ if(!state.subjectPreferences.order.includes(id)) state.subjectPreferences.order.push(id); });
  saveSubjectPreferences();
}
function sortSubjects(list){
  const orderMap=new Map();
  (state.subjectPreferences.order||[]).forEach((id,idx)=>orderMap.set(id,idx));
  const pinned=new Set(state.subjectPreferences.pinned||[]);
  return list.slice().sort((a,b)=>{
    const ap=pinned.has(a.id)?1:0, bp=pinned.has(b.id)?1:0;
    if(ap!==bp) return bp-ap;
    const ao=orderMap.has(a.id)?orderMap.get(a.id):Number.MAX_SAFE_INTEGER;
    const bo=orderMap.has(b.id)?orderMap.get(b.id):Number.MAX_SAFE_INTEGER;
...
function getGroupOrderKey(subjectName, sectionType){
  ...
  const finalType = normalized === 'lecture' ? 'lectures' : normalized === 'year' ? 'years' : normalized;
  return `${subjectName || 'unknown'}::${finalType || 'custom'}`;
}
function getOriginalGroupIds(subjectName, sectionType){
  const subject = state.subjects.find(s => s.name === subjectName || s.id === subjectName);
  if(!subject) return [];
  const normalized = String(sectionType || '').toLowerCase();
  if(normalized === 'lectures' || normalized === 'lecture') return (subject.lectures || []).map(g=>g.id);
  if(normalized === 'years' || normalized === 'year') return (subject.years || []).map(g=>g.id);
  if(normalized === 'ai') return (subject.ai || []).map(g=>g.id);
  return [];
}
function moveGroupToBottom(subjectName, sectionType, groupId){
  const key = getGroupOrderKey(subjectName, sectionType);
  const current = Array.isArray(state.groupPreferences[key]) ? state.groupPreferences[key].slice() : getOriginalGroupIds(subjectName, sectionType);
  const next = current.filter(id => id !== groupId);
  next.push(groupId);
  state.groupPreferences[key] = next;
  saveGroupPreferences();
}
function markLectureCompletedAndMoveBottom(group){
  if(!group || !group.id || !group.subjectName) return;
  if(!state.checklistCompleted[group.id]){
    state.checklistCompleted[group.id] = true;
    saveChecklistStore();
  }
  moveGroupToBottom(group.subjectName, group.type || 'lectures', group.id);
}
function updateAutoCompletedLectures(){
  for(const subject of state.subjects){
    for(const group of (subject.lectures || [])){
      const key = getGroupProgressKey('lecture', subject.name, group.name);
      const answered = getAnsweredCountForKey(key);
      const total = (group.questions || []).length;
      if(total > 0 && answered >= total){
        markLectureCompletedAndMoveBottom({
          id: group.id,
...
function applyThemeUI(){
  const t=theme();
  el('nav-icon-exams').textContent=t.icons.exams;
  el('nav-icon-wrong').textContent=t.icons.wrong;
  el('nav-icon-favorites').textContent=t.icons.favorites;
  if(el('nav-icon-checklist')) el('nav-icon-checklist').textContent=t.icons.checklist || '☑️';
  el('nav-icon-search').textContent=t.icons.search;
  el('nav-icon-statistics').textContent=t.icons.statistics;
  el('nav-icon-settings').textContent=t.icons.settings;
  if(el('nav-icon-memories')) el('nav-icon-memories').textContent='📖';
  if(el('statistics-screen-title')) el('statistics-screen-title').textContent=t.texts.statsTitle;
  if(el('settings-title')) el('settings-title').textContent=t.texts.settingsTitle;
  if(el('exam-settings-title')) el('exam-settings-title').textContent=t.texts.examSettingsTitle;
  if(el('btn-exam-settings')) el('btn-exam-settings').textContent=t.texts.examSettingsButton;
  if(el('btn-start-exam')) el('btn-start-exam').textContent=t.texts.startExam;
  const a=document.querySelector('#btn-training-mode .mode-label');
  const b=document.querySelector('#btn-exam-mode .mode-label');
  if(a) a.textContent=t.texts.trainingLabel;
  if(b) b.textContent=t.texts.examLabel;
}
function syncSettingsControls(){
  const entries=[
    ['dark-mode-toggle','checked',!!state.settings.darkMode],
    ['theme-selector','value',state.settings.theme],
    ['exam-theme-selector','value',state.settings.theme],
    ['sound-selector','value',state.settings.bgSound],
    ['exam-sound-selector','value',state.settings.bgSound],
    ['bg-sound-enabled-toggle','checked',state.settings.bgSoundEnabled!==false],
    ['exam-bg-sound-enabled-toggle','checked',state.settings.bgSoundEnabled!==false],
    ['volume-control','value',state.settings.volume],
    ['exam-volume-control','value',state.settings.volume],
    ['feedback-toggle','checked',state.settings.feedbackEnabled!==false],
    ['exam-feedback-toggle','checked',state.settings.feedbackEnabled!==false],
    ['animations-toggle','checked',state.settings.animations!==false]
  ];
  entries.forEach(([id,prop,val])=>{
    const x=el(id);
    if(x) x[prop]=val;
  });
}
async function resolveAssetPath(candidates){
  for(const item of candidates.filter(Boolean)){
    try{
      const u=encodeURI(item);
      const r=await fetch(u,{method:'HEAD'});
      if(r.ok) return u;
    }catch(e){}
  }
  return encodeURI(candidates.find(Boolean)||'');
}
async function applyBackgroundSound(userInitiated=false){
  const audio=el('bg-audio');
  if(!audio) return;
  audio.volume=(state.settings.volume||50)/100;
  const key=state.settings.bgSound||'none';
  const sound=BACKGROUND_SOUNDS[key]||BACKGROUND_SOUNDS.none;
  if(!state.settings.bgSoundEnabled || key==='none' || !sound.file){
    audio.pause();
    return;
  }
  const src=await resolveAssetPath([sound.file,'audio/'+sound.file,'assets/audio/'+sound.file]);
  if(audio.dataset.currentSrc!==src){
    audio.src=src;
    audio.dataset.currentSrc=src;
    audio.load();
  }
  if(state.audioUnlocked || userInitiated){
    state.audioUnlocked = true;
    audio.play().catch(()=>{});
  }
}
function applyEffectAudioVolumes(){
  ['right-audio','wrong-audio','celebrate-audio','seconds-audio'].forEach(id=>{
    const a=el(id);
...
async function prepareStaticEffectAudio(){
  ...
  const seconds = el('seconds-audio');
  ...
  if(seconds) seconds.src=await resolveAssetPath(['seconds.mp3','audio/seconds.mp3','assets/audio/seconds.mp3']);
...
function playEffectSound(kind){
  if(!state.currentExam || state.currentExam.mode!=='training' || state.settings.feedbackEnabled===false) return;
  const a=el(kind==='right'?'right-audio':'wrong-audio');
  if(!a||!a.src) return;
  try{ a.currentTime=0; a.play().catch(()=>{}); }catch(e){}
}
function playCelebrateSound(){
  if(state.settings.feedbackEnabled===false) return;
  const a=el('celebrate-audio');
  if(!a||!a.src) return;
  try{ a.currentTime=0; a.play().catch(()=>{}); }catch(e){}
}
function playSecondsWarning(){
  if(state.settings.feedbackEnabled===false) return;
  const a=el('seconds-audio');
  if(!a||!a.src) return;
  try{ a.currentTime=0; a.play().catch(()=>{}); }catch(e){}
}
...
function changeTheme(name){
  ...
  applySettings();
  renderSubjects();
  if(state.currentSubject && el('subject-sections-screen').classList.contains('active')) openSubject(state.currentSubject.id);
  if(state.currentExam && el('exam-screen').classList.contains('active')) renderExam();
  renderStatisticsPage();
  renderChecklist();
  if(el('checklist-subject-screen') && el('checklist-subject-screen').classList.contains('active')) renderChecklistSubject();
  if(el('subject-stats-screen') && el('subject-stats-screen').classList.contains('active')) renderSubjectStats();
  renderMemories();
}
function changeSound(name){
  state.settings.bgSound = BACKGROUND_SOUNDS[name] ? name : 'none';
  saveSettings();
  applySettings();
}
function changeVolume(v){
  state.settings.volume=clampNum(parseInt(v,10),0,100,50);
  saveSettings();
  applySettings();
}
function toggleDarkMode(){
  state.settings.darkMode=!!el('dark-mode-toggle').checked;
  saveSettings();
  applySettings();
}
function toggleBackgroundSoundEnabled(){
  const examToggle = el('exam-bg-sound-enabled-toggle');
  const homeToggle = el('bg-sound-enabled-toggle');
  let checked = false;
  if(document.activeElement && (document.activeElement.id==='exam-bg-sound-enabled-toggle' || document.activeElement.id==='bg-sound-enabled-toggle')){
    checked = !!document.activeElement.checked;
  } else if(examToggle && el('exam-screen') && el('exam-screen').classList.contains('active')){
    checked = !!examToggle.checked;
  } else if(homeToggle){
    checked = !!homeToggle.checked;
  }
  state.settings.bgSoundEnabled=checked;
  state.audioUnlocked = state.audioUnlocked || isIOSDevice() || !!el('exam-screen')?.classList.contains('active');
  saveSettings();
  syncSettingsControls();
  applyBackgroundSound(true);
}
function toggleFeedbackSounds(){
  const examToggle = el('exam-feedback-toggle');
  const homeToggle = el('feedback-toggle');
  let checked = false;
  if(document.activeElement && (document.activeElement.id==='exam-feedback-toggle' || document.activeElement.id==='feedback-toggle')){
    checked = !!document.activeElement.checked;
  } else if(examToggle && el('exam-screen') && el('exam-screen').classList.contains('active')){
    checked = !!examToggle.checked;
  } else if(homeToggle){
  ...
  state.settings.feedbackEnabled=checked;
  state.audioUnlocked = state.audioUnlocked || isIOSDevice() || !!el('exam-screen')?.classList.contains('active');
  saveSettings();
  syncSettingsControls();
}
function toggleAnimations(){
  state.settings.animations=!!el('animations-toggle').checked;
  saveSettings();
  applySettings();
}
function toggleSettings(){ el('settings-panel').classList.toggle('visible'); }
function toggleExamSettings(show){
  el('exam-settings-modal').classList.toggle('hidden', !show);
  if(show) syncSettingsControls();
}

function removeDialogExtraButtons(){
...
function appendDialogExtraButton({text,className='btn-secondary dialog-extra-btn',onClick,prepend=false}){
  const actions = document.querySelector('#dialog-overlay .dialog-actions');
  ...
  btn.onclick = onClick;
  if(prepend && actions.firstChild) actions.insertBefore(btn, actions.firstChild);
...
function showToast(message, kind='info', duration=2600){
  ...
  const prefix=(theme().icons[kind] || theme().icons.statistics || 'ℹ️');
...
function showDialog({title='تنبيه', message='', showCancel=false, confirmText='موافق', cancelText='إلغاء', onConfirm=null, onCancel=null}){
  removeDialogExtraButtons();
  el('dialog-title').textContent=title;
  el('dialog-body').innerHTML=message;
  const cancel=el('dialog-cancel');
  const confirm=el('dialog-confirm');
...
function isSectionExcluded(sectionType){
...
function setEmptyText(text, icon='📁'){
  if(el('subjects-empty-text')) el('subjects-empty-text').textContent=text;
  const empty=el('subjects-empty');
  const ic=empty?empty.querySelector('.empty-icon'):null;
...
function moveSubject(subjectId,direction){
  ...
  const arr=state.subjectPreferences.order.slice();
  const idx=arr.indexOf(subjectId);
  if(idx===-1) return;
  const target=direction==='up'?idx-1:idx+1;
  if(target<0||target>=arr.length) return;
  [arr[idx],arr[target]]=[arr[target],arr[idx]];
  state.subjectPreferences.order=arr;
  saveSubjectPreferences();
...
function togglePinSubject(subjectId){
  ...
  state.subjectPreferences.pinned=Array.from(pinned);
  saveSubjectPreferences();
  state.subjects=sortSubjects(state.subjects);
  renderSubjects();
  openSubjectActions(subjectId);
...
function openSubjectActions(subjectId){
  state.openSubjectActionId=subjectId;
  document.querySelectorAll('.subject-card').forEach(card=>card.classList.toggle('actions-open', card.getAttribute('data-subject-id')===subjectId));
}
function closeSubjectActions(){
  state.openSubjectActionId=null;
...
function startSubjectLongPress(subjectId){
  if(state.browseMode!=='all') return;
  clearTimeout(state.longPressTimer);
  state.longPressTimer=setTimeout(()=>openSubjectActions(subjectId),420);
}
...
function handleSubjectOpen(subjectId,event){
  if(event && event.target && event.target.closest('.subject-action-btn')) return;
  if(state.openSubjectActionId===subjectId) return;
...
function renderChecklist(){
  ...
  const subjects = sortSubjects(state.subjects);
  if(!subjects.length){
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">📭</div><p>لا توجد مواد بعد.</p></div>';
    return;
  }
...
function openChecklistSubject(subjectId){
  const subject = state.subjects.find(s => s.id === subjectId);
  if(!subject) return;
  state.currentSubject = subject;
...
function renderChecklistSubject(){
  ...
  if(el('checklist-subject-lectures')) el('checklist-subject-lectures').innerHTML = subject.lectures.length ? subject.lectures.map(group => {
    const checked = !!state.checklistCompleted[group.id];
    return `<label class="checklist-lecture-row"><input type="checkbox" ${checked?'checked':''} onchange="toggleChecklistLecture('${group.id}')"><span class="checklist-lecture-name ${checked?'completed':''}">${escapeHtml(group.name)}</span><span class="checklist-lecture-count">${group.questions.length} سؤال</span></label>`;
...
function toggleChecklistLecture(groupId){
  state.checklistCompleted[groupId] = !state.checklistCompleted[groupId];
  saveChecklistStore();
  renderChecklist();
  if(el('checklist-subject-screen') && el('checklist-subject-screen').classList.contains('active')) renderChecklistSubject();
  updateStatisticsIfOpen();
...
function openSubject(subjectId){
  state.currentSubject=state.displayedSubjects.find(s=>s.id===subjectId) || state.subjects.find(s=>s.id===subjectId) || null;
  if(!state.currentSubject){ showToast('المادة غير موجودة.','error'); return; }
  const t=theme();
  el('subject-sections-title').textContent=state.currentSubject.name;
  el('subject-sections-summary').innerHTML=`<div class="subject-summary-grid"><div><span>${t.icons.lectures} المحاضرات</span><strong>${state.currentSubject.lectures.length}</strong></div><div><span>${t.icons.years} Years</span><strong>${state.currentSubject.years.length}</strong></div><div><span>${t.icons.ai} AI</span><strong>${state.currentSubject.ai.length}</strong></div><div><span>${t.icons.progress} إجمالي الأسئلة</span><strong>${state.currentSubject.totalQuestions}</strong></div></div>`;
  const cards=[];
  if(state.currentSubject.lectures.length) cards.push(buildCategoryCard('lectures','Lectures','Lectures',state.currentSubject.lectures.length,countQuestions(state.currentSubject.lectures),true));
  if(state.currentSubject.years.length) cards.push(buildCategoryCard('years','Years','Years',state.currentSubject.years.length,countQuestions(state.currentSubject.years),true));
  if(state.currentSubject.ai.length || (state.browseMode==='all' && state.currentSubject.hasAiFolder)) cards.push(buildCategoryCard('ai','AI','AI',state.currentSubject.ai.length,countQuestions(state.currentSubject.ai),state.currentSubject.ai.length>0 || (state.browseMode==='all' && state.currentSubject.hasAiFolder)));
  el('subject-categories').innerHTML=cards.join('') || '<div class="empty-state"><div class="empty-icon">📭</div><p>لا توجد ملفات TXT بعد داخل هذه المادة.</p></div>';
  showScreen('subject-sections-screen');
}
function countQuestions(groups){ return groups.reduce((sum,g)=>sum+g.questions.length,0); }
function buildCategoryCard(type,title,badgeText,itemCount,totalQuestions,enabled){
  if(!enabled) return '';
  return `<button class="category-card" onclick="openSubjectCategory('${type}')"><span class="category-badge">${badgeText}</span><div class="category-meta"><div><span>المحاضرات</span><strong>${itemCount}</strong></div><div><span>الأسئلة</span><strong>${totalQuestions}</strong></div></div></button>`;
}
function openSubjectCategory(type){
  if(!state.currentSubject) return;
  let groups=[], title='', placeholder='ابحث...';
  if(type==='lectures'){
...
function showSelectionScreen(groups,title,meta){
  ...
  state.extraTime=0;
  state.extraTimeAdded=false;
  state.currentSelectionMeta=meta||{};
  showScreen('selection-screen');
  el('selection-title').textContent=title;
  const searchContainer=el('selection-search-container');
  const searchInput=el('selection-search');
  ...
  el('selection-footer').classList.add('hidden');
  el('direction-selection').classList.add('hidden');
  el('timer-options').classList.add('hidden');
  el('start-section').classList.add('hidden');
...
function filterSelectionList(){
  const term=String(el('selection-search').value||'').toLowerCase().trim();
...
function toggleGroupSelection(idx){
  const existing=state.selectedGroups.indexOf(idx);
  if(existing>-1) state.selectedGroups.splice(existing,1); else state.selectedGroups.push(idx);
  document.querySelectorAll('#selection-list .selection-item').forEach((item,itemIdx)=>item.classList.toggle('selected', state.selectedGroups.includes(itemIdx)));
  updateSelectionFooter();
}
function updateSelectionFooter(){
  const footer=el('selection-footer');
  const totalQuestions=state.selectedGroups.reduce((sum,idx)=>sum+state.currentGroups[idx].questions.length,0);
  if(state.selectedGroups.length>0){
    footer.classList.remove('hidden');
    el('selected-count').textContent=totalQuestions+' questions selected';
    const input=el('question-count-input');
    input.max=totalQuestions;
    input.value=totalQuestions;
    el('max-questions-label').textContent='/ '+totalQuestions;
...
function selectMode(mode){
  ...
  state.extraTimeAdded=false;
  document.querySelectorAll('.btn-mode').forEach(btn=>btn.classList.remove('active'));
  el(mode==='training'?'btn-training-mode':'btn-exam-mode').classList.add('active');
  document.querySelectorAll('.btn-direction').forEach(btn=>btn.classList.remove('active'));
  el('direction-selection').classList.remove('hidden');
  el('timer-options').classList.add('hidden');
  el('start-section').classList.add('hidden');
}
function selectDirection(direction){
  ...
  document.querySelectorAll('.btn-direction').forEach(btn=>btn.classList.remove('active'));
  el(direction==='oneway'?'btn-oneway':'btn-twoway').classList.add('active');
  if(state.selectedMode==='exam'){
    const count=parseInt(el('question-count-input').value,10)||0;
    el('base-time-display').textContent=count+' min';
    el('extra-time-display').textContent='+0 min';
    el('total-time-display').textContent=count+' min';
    el('timer-options').classList.remove('hidden');
    const btn=el('btn-add-extra');
    btn.disabled=false;
    btn.textContent='+ إضافة 5 دقائق';
    btn.classList.remove('active');
    state.extraTime=0;
    state.extraTimeAdded=false;
  } else el('timer-options').classList.add('hidden');
  el('start-section').classList.remove('hidden');
}
function addExtraTime(){
  const count=parseInt(el('question-count-input').value,10)||0;
  const btn=el('btn-add-extra');
  if(!state.extraTimeAdded){
    state.extraTime=5;
    state.extraTimeAdded=true;
    el('extra-time-display').textContent='+5 min';
    el('total-time-display').textContent=(count+5)+' min';
    btn.disabled=false;
    btn.textContent='− إزالة 5 دقائق';
...
function hasIncompleteSavedExam(){
  const raw=localStorage.getItem(STORAGE_KEYS.examState);
  if(!raw) return false;
  try{
    const saved=JSON.parse(raw);
    return !!(saved && !saved.submitted && Array.isArray(saved.questions) && saved.questions.length);
  }catch{
    return false;
  }
}
...
function promptResumeIncompleteExam(onDiscardAndContinue){
  showDialog({
    title:'يوجد امتحان غير مكتمل',
    message:'هل تريد المتابعة من حيث توقفت؟',
    showCancel:true,
    confirmText:'نعم أريد إكمال الامتحان',
    cancelText:'لا (ستفقد بيانات التقدم لهذا الامتحان)',
    onConfirm:()=>resumeSavedExam(),
    onCancel:()=>{
      clearExamState();
      if(typeof onDiscardAndContinue==='function') onDiscardAndContinue();
    }
  });
}
function internalStartExamFlow(){
  try{
    if(!state.selectedMode || !state.selectedDirection){
      showToast('يرجى اختيار النمط واتجاه التنقل.','error');
      return;
    }
    const count=parseInt(el('question-count-input').value,10);
    if(!count || count<1){
      showToast('يرجى إدخال عدد صحيح من الأسئلة.','error');
      return;
    }
    let questions=[];
    state.selectedGroups.forEach(idx=>{
      if(state.currentGroups[idx] && Array.isArray(state.currentGroups[idx].questions)) questions=questions.concat(state.currentGroups[idx].questions);
    });
    if(!questions.length){
      showToast('لا توجد أسئلة متاحة للبدء.','error');
      return;
    }
    const selectedGroupObjects=state.selectedGroups.map(idx=>state.currentGroups[idx]).filter(Boolean);
    const historySubjectName=selectedGroupObjects[0]?.subjectName || state.currentSubject?.name || 'Unknown Subject';
    const historyGroups=selectedGroupObjects.map(g=>({ name:g.name, type:g.type || state.currentSelectionMeta?.sectionType || 'lecture' }));
    const randomizedQuestions = shuffleArray(questions).slice(0, count).map(prepareQuestionForExam);
    startExamSession(randomizedQuestions, state.selectedMode, state.selectedDirection, state.currentSelectionMeta ? state.currentSelectionMeta.sectionType : 'custom', state.extraTime, { collectionType: state.currentSelectionMeta ? state.currentSelectionMeta.collectionType : null, displayLabel: state.currentSelectionMeta ? state.currentSelectionMeta.displayLabel : 'Exam', historySubjectName, historyGroups });
  }catch(err){
    console.error('confirmStartExam error', err);
    showToast('حدث خطأ عند بدء الامتحان. افتح Console إن استمرت المشكلة.','error');
  }
}
function confirmStartExam(){
  if(hasIncompleteSavedExam() && !(state.currentExam && el('exam-screen') && el('exam-screen').classList.contains('active'))){
    promptResumeIncompleteExam(()=>internalStartExamFlow());
    return;
  }
  internalStartExamFlow();
}
function prepareQuestionForExam(question){
  ...
  clone.originalQuestionNumber = clone.number || '';
...
function renderRemoveWrongBtn(){
  if(!state.currentExam || state.currentExam.collectionType!=='wrong' || state.currentExam.mode!=='training') return '';
  const idx=state.currentExam.currentIndex;
  const q=state.currentExam.questions[idx];
  const answered=state.currentExam.firstAnswers[idx];
  if(answered===null || !isAnswerCorrect(q, answered) || !state.wrongQuestions.includes(q.id)) return '';
  return `<button class="btn-secondary mt-20" onclick="confirmRemoveCurrentWrong()">✅ إزالة السؤال من الأخطاء</button>`;
}
function startSpecialExam(questions, mode, direction){
  startExamSession(shuffleArray(questions.slice()).map(prepareQuestionForExam), mode, direction||'twoway', 'special', 5, { collectionType:null, displayLabel:'Special Exam', historySubjectName:'Special Exam', historyGroups:[] });
}

function renderOptionButton(opt, i, idx, showAnswerState, selectedIndex, correctIdx){
  let cls='option-btn';
...
function renderExam(){
  ...
  const answeredInTraining = state.currentExam.firstAnswers.filter(x=>x!==null).length;
  const correctInTraining = state.currentExam.firstAnswers.filter((ans,i)=>ans!==null && isAnswerCorrect(questions[i],ans)).length;
  const pctTraining = answeredInTraining>0 ? Math.round((correctInTraining/answeredInTraining)*100) : 0;
  let progressText = '';
  if(state.currentExam.mode==='training'){
    progressText = `🎯 ${idx+1}/${questions.length} · ✅${correctInTraining} · ${pctTraining}%`;
  } else {
    progressText = `🎯 ${idx+1}/${questions.length}`;
  }
...
function renderGrid(){
  ...
  state.currentExam.questions.forEach((q,idx)=>{
    let cls='grid-btn';
    if(idx===state.currentExam.currentIndex) cls+=' current';
    else if(state.currentExam.answers[idx]!==null){
      if(state.currentExam.mode==='training' && state.currentExam.firstAnswers[idx]!==null) cls+=isAnswerCorrect(q,state.currentExam.firstAnswers[idx])?' answered':' wrong';
      else cls+=' answered';
    }
    if(state.currentExam.direction==='oneway' && idx<state.currentExam.currentIndex) cls+=' disabled';
    const btn=document.createElement('button');
    btn.className=cls;
...
function renderExamNav(){
  ...
  let prevBtn='<span></span>', nextBtn='<span></span>';
  if(state.currentExam.direction==='twoway' && idx>0) prevBtn='<button class="btn-secondary" onclick="prevQuestion()">Previous ←</button>';
  if(state.currentExam.mode==='training'){
    if(state.currentExam.showAnswer) nextBtn=idx<last?'<button class="btn-primary" onclick="nextQuestion()">Next →</button>':'<button class="btn-primary" onclick="finishExam()">Finish</button>';
    else if(state.currentExam.answers[idx]!==null) nextBtn='<button class="btn-small" onclick="showAnswer()">Show Answer</button>';
  } else if(state.currentExam.answers[idx]!==null) nextBtn=idx<last?'<button class="btn-primary" onclick="nextQuestion()">Next →</button>':'<button class="btn-primary" onclick="finishExam()">Finish</button>';
...
function prevQuestion(){
  ...
  if(state.currentExam.currentIndex>0){
    state.currentExam.currentIndex-=1;
    if(state.currentExam.mode==='training') state.currentExam.showAnswer=state.currentExam.answers[state.currentExam.currentIndex]!==null;
    saveExamState();
    renderExam();
    scrollQuestionIntoView(true);
  }
...
function navigateToQuestion(index){
  ...
  if(state.currentExam.mode==='training') state.currentExam.showAnswer=state.currentExam.answers[index]!==null;
  saveExamState();
  renderExam();
  scrollQuestionIntoView(true);
}
function scrollQuestionIntoView(smooth){
  const q=el('question-container');
  if(!q) return;
  const top=q.getBoundingClientRect().top + window.scrollY - 12;
  window.scrollTo({top, behavior:smooth && state.settings.animations!==false ? 'smooth':'auto'});
}
function toggleQuestionLocation(){
  if(!state.currentExam) return;
  const q=state.currentExam.questions[state.currentExam.currentIndex];
  const parts=[];
  if(q.subjectName) parts.push('المادة: '+q.subjectName);
  if(q.lectureName) parts.push('الملف: '+q.lectureName);
  if(q.batchName) parts.push('الدفعة: '+q.batchName);
  if(q.originalQuestionNumber) parts.push('رقم السؤال الأصلي: '+q.originalQuestionNumber);
  if(q.pageNumber) parts.push('الصفحة: '+q.pageNumber);
  showToast(parts.join(' | ') || 'لا توجد بيانات موقع متاحة.','info',3600);
}
function confirmRemoveCurrentWrong(){
  if(!state.currentExam || state.currentExam.collectionType!=='wrong') return;
  const q=state.currentExam.questions[state.currentExam.currentIndex];
  askConfirm('هل أتقنت هذا السؤال وتريد إزالته من الأسئلة الخاطئة؟', ()=>removeWrongQuestionsByIds([q.id]));
}
function removeWrongQuestionsByIds(ids){
  const set=new Set(ids);
  state.wrongQuestions = state.wrongQuestions.filter(id=>!set.has(id));
  saveWrongQuestions();
  updateStatisticsIfOpen();
  if(state.currentExam){
    state.currentExam.masteredWrongIds = state.currentExam.masteredWrongIds.filter(id=>!set.has(id));
    renderExam();
  }
  if(state.browseMode==='wrong') renderSubjects();
  showToast('تمت إزالة السؤال/الأسئلة من قائمة الأخطاء.','success');
}
function toggleGrid(){
  const grid=el('question-grid');
  const btn=el('btn-grid-toggle');
  grid.classList.toggle('hidden');
  btn.innerHTML=grid.classList.contains('hidden')?'<span>☰</span> إظهار الشبكة':'<span>☰</span> إخفاء الشبكة';
}

function exitExamWithoutSaving(){
...
function exitExamWithSaving(){
  if(state.currentExam) saveExamState();
  state.currentExam=null;
  clearInterval(state.timerInterval);
  state.timerInterval=null;
  goHome();
...
function exitExam(){
  ...
  showDialog({
    ...
    onConfirm:()=>exitExamWithSaving(),
    onCancel:()=>{}
  });
  appendDialogExtraButton({
    text:'نعم وعدم حفظ التقدم',
    className:'btn-danger dialog-extra-btn',
    prepend:true,
    onClick:()=>{ hideDialog(); exitExamWithoutSaving(); }
  });
}
...
function startTimer(){
  ...
  state.timerInterval=setInterval(()=>{
    ...
    if(remaining<=11000 && !state.timerWarningPlayed){
...
function recordExamMemory(){
  if(!state.currentExam) return;
  const end = state.currentExam.endTime || Date.now();
  const start = state.currentExam.startTime || end;
  const answers = state.currentExam.mode==='exam' ? state.currentExam.answers : state.currentExam.firstAnswers;
  state.currentExam.questions.forEach((q,idx)=>{
    if(answers[idx]===null) return;
    const existing = state.questionsFirstSeen[q.id];
    const newDay = getDayKey(end);
    const oldDay = existing ? getDayKey(existing.ts || 0) : '';
    if(!existing || oldDay !== newDay){
      state.questionsFirstSeen[q.id] = { ts:end, subjectName:q.subjectName || 'Unknown' };
    }
  });
  ...
  if(state.currentExam.collectionType==='wrong'){
    state.currentExam.questions.forEach((q,idx)=>{
      ...
      if(ans!==null && isAnswerCorrect(q,ans) && state.wrongQuestions.includes(q.id)) mastered.push(q.id);
    });
  ...
  state.examHistory.push({
    ...
    sourceLabel:state.currentExam.displayLabel || state.currentExam.sourceLabel || 'Exam',
    collectionType:state.currentExam.collectionType || null,
    total:stats.total,
    correct:stats.correct,
    score:stats.score,
    subjectName: state.currentExam.historySubjectName || (state.currentExam.questions[0]?.subjectName || 'Unknown Subject'),
    groups: state.currentExam.historyGroups || []
  });
  saveMemoryStores();
}
function finalizeFinishExam(){
  if(!state.currentExam) return;
...
function finishExam(){
  if(!state.currentExam) return;
  if(state.currentExam.mode==='training'){
    const unanswered = state.currentExam.firstAnswers.filter(a=>a===null).length;
    if(unanswered>0){
      showDialog({
        title:'تأكيد التسليم',
        message:`هناك ${unanswered} سؤال لم تحلهم ، هل تريد تأكيد تسليم الاختبار دون حلهم ؟`,
        showCancel:true,
        confirmText:'نعم',
        cancelText:'لا ، العودة للامتحان',
        onConfirm:()=>finalizeFinishExam(),
        onCancel:()=>{}
      });
      return;
    }
...
function showWaitingMessages(){
  const waitDiv=el('results-waiting'), contentDiv=el('results-content'), reviewDiv=el('results-review'), messageEl=el('waiting-message');
  const messages=['انتظر ...','قاعد بصلّح لك الامتحان ...','استنى شوي ...','هاي قرّبت أكمل ...','أصبر ثواني ...','هيني كمّلت 🫡🐦‍🔥💚'];
...
function calculateScore(exam){
  ...
  const correct=answers.reduce((sum,a,idx)=>sum + (a!==null && isAnswerCorrect(questions[idx],a) ? 1 : 0),0);
...
function renderMasteredWrongButton(){
  if(!state.currentExam || !state.currentExam.masteredWrongIds || !state.currentExam.masteredWrongIds.length) return '';
  return `<button class="btn-secondary mt-10" onclick="confirmBulkRemoveMasteredWrong()">✅ إزالة الأسئلة التي تمكنت منها من الأسئلة الخاطئة</button>`;
}
function confirmBulkRemoveMasteredWrong(){
  if(!state.currentExam || !state.currentExam.masteredWrongIds || !state.currentExam.masteredWrongIds.length) return;
  askConfirm('سيتم إزالة كل الأسئلة التي أجبت عنها بشكل صحيح في هذا الامتحان من قائمة الأسئلة الخاطئة. هل تريد المتابعة؟', ()=>removeWrongQuestionsByIds(state.currentExam.masteredWrongIds.slice()));
}
function showResults(){
  if(!state.currentExam) return;
  const t=theme();
  showScreen('results-screen');
  el('results-title').textContent=t.icons.results+' '+t.texts.resultsTitle;
  const stats=calculateScore(state.currentExam);
  const timeSpent=state.currentExam.endTime ? Math.round((state.currentExam.endTime-state.currentExam.startTime)/1000) : 0;
  const mins=Math.floor(timeSpent/60), secs=timeSpent%60;
  if(stats.score>=50){
    playCelebrateSound();
    if(state.settings.animations!==false) showFireworks(110,24);
  }
  el('results-content').innerHTML=`<div class="result-score">${stats.score}%</div><div class="result-details"><div class="result-card"><div class="value">${stats.correct}/${stats.total}</div><div class="label">Correct</div></div><div class="result-card"><div class="value">${mins}m ${secs}s</div><div class="label">Time Spent</div></div><div class="result-card"><div class="value">${stats.unanswered}</div><div class="label">Unanswered</div></div><div class="result-card"><div class="value">${stats.incorrect}</div><div class="label">Incorrect</div></div></div><button class="btn-primary mt-20" onclick="reviewExam()">${t.icons.review} Review Questions</button>${renderMasteredWrongButton()}<button class="btn-secondary mt-10" onclick="goHome()">${t.icons.exams} Back to Home</button>`;
}
function reviewExam(){
  if(!state.currentExam) return;
...
function populateSearchFilter(){
  const filter=el('search-filter');
  if(!filter) return;
  filter.innerHTML='<option value="all">All Subjects</option>';
  state.subjects.forEach(s=>{ filter.innerHTML += '<option value="'+escapeAttribute(s.id)+'">'+escapeHtml(s.name)+'</option>'; });
}
function performSearch(){
  const query=String(el('search-input').value||'').toLowerCase().trim();
  const filter=el('search-filter').value;
...
function showLocation(subjectName, lectureName, batchName, questionNumber, pageNumber){
  const parts=[];
  if(subjectName) parts.push('المادة: '+subjectName);
  if(lectureName) parts.push('الملف: '+lectureName);
  if(batchName) parts.push('الدفعة: '+batchName);
  if(questionNumber) parts.push('رقم السؤال الأصلي: '+questionNumber);
  if(pageNumber) parts.push('الصفحة: '+pageNumber);
  showToast(parts.join(' | ') || 'لا توجد بيانات موقع متاحة.','info',3600);
}
function toggleFavorite(questionId){
  const idx=state.favorites.indexOf(questionId);
  if(idx>-1) state.favorites.splice(idx,1); else state.favorites.push(questionId);
  saveFavorites();
  if(state.currentExam && !state.currentExam.submitted) renderExam();
  if(el('readonly-screen').classList.contains('active')) openReadonly(questionId);
  if(state.browseMode==='favorites') renderSubjects();
  updateStatisticsIfOpen();
}
...
function getAnsweredCountForKey(key){
  ...
  return new Set(entry.questionIds||[]).size;
}
function getSectionSummary(subject, type){
  const groups = type==='lecture' ? subject.lectures : type==='ai' ? subject.ai : subject.years;
  const totalQuestions = groups.reduce((sum,g)=>sum+g.questions.length,0);
  let answered = 0;
  groups.forEach(g=>answered += getAnsweredCountForKey(getGroupProgressKey(type, subject.name, g.name)));
  const pct = totalQuestions ? Math.round((answered/totalQuestions)*100) : 0;
  return { groups, totalQuestions, answered, pct };
}
function resetProgressFull(){
  askConfirm('هل تريد إعادة ضبط جميع البيانات (جميع المواد)؟ لا يمكن التراجع عن هذا الإجراء.', ()=>{
    state.progress={};
    saveProgressStore();
    showToast('تمت إعادة ضبط جميع بيانات التقدم.','success');
    updateStatisticsIfOpen();
    renderMemories();
  });
}
...
function getMemoryDateRange(period, offset){
  const now = new Date();
  if(period==='weekly'){
    const anchor = getStartOfWeek(now);
    anchor.setDate(anchor.getDate() + ((offset||0) * 7));
  ...
  if(period==='monthly'){
    ...
    const end = getEndOfMonth(anchor);
    return {start,end};
  }
  const anchor = new Date(now.getFullYear() + (offset||0), 0, 1);
  const start = getStartOfYear(anchor);
  const end = getEndOfYear(anchor);
  return {start,end};
}
function aggregateMemorySeries(period, detailed, selectedSubjects){
  ...
  const offset = state.memoryOffset[period] || 0;
  const range = getMemoryDateRange(period, offset);
  let labels=[]; let buckets=[]; let caption='';
  if(period==='weekly'){
    caption=`من ${range.start.toLocaleDateString('ar-EG')} إلى ${range.end.toLocaleDateString('ar-EG')}`;
    for(let i=0;i<7;i++){
...
function drawMemoriesChart(canvas, labels, series){
  const ctx=canvas.getContext('2d');
  const w=canvas.width, h=canvas.height;
  ctx.clearRect(0,0,w,h);
  ctx.fillStyle='rgba(255,255,255,0.0)';
  ctx.fillRect(0,0,w,h);
  const left=70, right=20, top=20, bottom=70;
  const allValues = series.flatMap(s=>s.data);
  const maxVal = Math.max(1, ...allValues.map(v => Math.round(v)));
  ...
  ctx.strokeStyle='rgba(100,116,139,.9)';
  ctx.beginPath();
  ctx.moveTo(left, top);
  ...
  for(let i=0;i<5;i++){
    ...
    const y = top + (h-top-bottom)*i/4 + 4;
    ctx.fillText(String(val), left-10, y);
  }
  ...
  series.forEach(s=>{
    ...
    s.data.forEach((value, i)=>{
      ...
      const x=left + step*i;
      const y=h-bottom - (num/maxVal)*plotH;
  ...
  series.forEach((s, idx)=>{
    ...
    ctx.fillStyle=s.color;
    ctx.fillRect(x, legendY-10, 18, 4);
    ctx.fillStyle='#475569';
    ctx.fillText(s.name, x+24, legendY-6);
  });
}
function shiftMemoriesRange(delta){
  const period = el('memory-period') ? el('memory-period').value : 'weekly';
  state.memoryOffset[period] = (state.memoryOffset[period] || 0) + delta;
  renderMemories();
}
...
function getFilteredExamHistory(){
  const selected=getCurrentHistoryFilterValue();
  return state.examHistory.slice().sort((a,b)=>(b.endedAt||0)-(a.endedAt||0)).filter(exam=>selected==='all' || exam.subjectName===selected);
}
function getHistoryItemsForDeletion(){
  const scope=el('history-delete-scope')?.value || 'visible';
  if(scope==='all') return state.examHistory.slice().sort((a,b)=>(b.endedAt||0)-(a.endedAt||0));
  return getFilteredExamHistory();
}
...
function confirmDeleteHistoryItems(){
  const selected=Array.from(document.querySelectorAll('.history-delete-checkbox:checked')).map(cb=>cb.value);
  const totalItems=state.examHistory.length;
  const deleteAll=selected.length && selected.length===totalItems;
  if(!selected.length){
    showToast('لم يتم تحديد أي عنصر للحذف.','error');
    return;
  }
  askConfirm(deleteAll ? 'سيتم حذف كل السجل ولن تستطيع التراجع. هل تريد المتابعة؟' : 'سيتم حذف المحدد ولن تستطيع التراجع. هل تريد المتابعة؟', ()=>{
    state.examHistory = state.examHistory.filter(item=>!selected.includes(item.id));
    saveMemoryStores();
    toggleHistoryDeleteModal(false);
    renderMemories();
    showToast(deleteAll ? 'تم حذف كل السجل.' : 'تم حذف المحدد من السجل.','success');
  });
}
function openMemories(){ renderMemories(); showScreen('memories-screen'); }
function switchMemoriesTab(tab){
  const chartTab=el('memories-chart-tab');
  const timeTab=el('memories-time-tab');
  const btnChart=el('mem-tab-chart');
  const btnTime=el('mem-tab-time');
  if(tab==='chart'){
...
function renderMemories(){
  ...
  if(filters){
    ...
    filters.innerHTML = subjects.length ? subjects.map(name=>`<label class="memory-subject-chip"><input type="checkbox" value="${escapeAttribute(name)}" ${selection.includes(name)?'checked':''} onchange="renderMemories()"> <span>${escapeHtml(name)}</span></label>`).join('') : '<div class="stats-empty-note">لا توجد مواد بعد.</div>';
  }
...
function showFireworks(durationFrames=80, explosionCount=18){
  ...
  const palettes={default:['#2563eb','#38bdf8','#10b981','#f59e0b'],desert:['#f4c95d','#db9b41','#9a5b24','#fff0c9'],space:['#8b5cf6','#38bdf8','#f0abfc','#ffffff'],pirates:['#d9a740','#7d4b16','#fdf0c2','#7ec8e3'],castle:['#d4af37','#7c3f98','#f5f1dc','#9ea7d8'],lab:['#00bcd4','#7ef9ff','#15d3a2','#ffffff']};
  const colors=palettes[state.settings.theme]||palettes.default;
  const particles=[];
  const bursts=[{x:canvas.width*.3,y:canvas.height*.35},{x:canvas.width*.7,y:canvas.height*.4},{x:canvas.width*.5,y:canvas.height*.26}];
  bursts.forEach(b=>{
    for(let i=0;i<explosionCount;i++){
      const angle=(Math.PI*2*i)/explosionCount;
      const speed=2.5+Math.random()*4.8;
      particles.push({x:b.x,y:b.y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,color:colors[Math.floor(Math.random()*colors.length)],size:Math.random()*3+1.5,life:1});
    }
  });
  let frame=0;
  function animate(){
...
function goHome(){
  const wasResults = el('results-screen').classList.contains('active');
  ...
  if(wasResults){ setTimeout(()=>softReloadApp(), 150); }
}
function softReloadApp(){
  const currentAudioTime = el('bg-audio') ? el('bg-audio').currentTime : 0;
  renderSubjects();
  updateStatisticsIfOpen();
  renderChecklist();
  renderMemories();
  if(el('bg-audio') && state.settings.bgSoundEnabled!==false && state.settings.bgSound!=='none'){
    try{
      el('bg-audio').currentTime = currentAudioTime;
      el('bg-audio').play().catch(()=>{});
    }catch(e){}
  }
}
...
function parseQuestionFile(raw, meta){
  ...
  for(let blockIndex=0; blockIndex<blocks.length; blockIndex++){
    ...
    let questionNumber=''; let questionText=''; let options=[]; let correctAnswer=''; let explanation=''; let batchName=''; let pageNumber=''; let startIndex=0;
    ...
    const correctAnswerText = (()=>{ const possibleIndex = resolveCorrectIndex(options, correctAnswer); if(possibleIndex>=0 && options[possibleIndex]) return options[possibleIndex]; return stripOptionPrefix(correctAnswer); })();
    ...
    questions.push({id,number:questionNumber,text:questionText,options,originalOptions:options.slice(),correctAnswer,correctAnswerText,correctIndex:resolveCorrectIndex(options,correctAnswerText),explanation,batchName,pageNumber,subjectName:meta.subjectName,subjectId:meta.subjectId||slugify(meta.subjectName),lectureName:meta.lectureName,groupName:meta.lectureName,sourceType:meta.sourceType,sourcePath:meta.sourcePath});
    fallback++;
  }
  return questions;
}
...
async function buildLectureGroupFromFile(fileItem, subjectName, subjectId, sourceType, startCounter){
  ...
  const questions=parseQuestionFile(text,{subjectName,subjectId,lectureName,sourceType,sourcePath:fileItem.path,startCounter});
...
async function scanSubjectFolder(dirItem){
  const items=await listRepoDirectory(dirItem.path);
  const lectureFiles=items.filter(it=>it.type==='file' && it.name.toLowerCase().endsWith('.txt')).sort((a,b)=>a.name.localeCompare(b.name));
  const aiFolder=items.find(it=>it.type==='dir' && it.name.toLowerCase()==='ai');
  let aiFiles=[];
  if(aiFolder){
    const aiItems=await listRepoDirectory(aiFolder.path);
    aiFiles=aiItems.filter(it=>it.type==='file' && it.name.toLowerCase().endsWith('.txt')).sort((a,b)=>a.name.localeCompare(b.name));
  }
  const subjectId = slugify(dirItem.name);
  if(lectureFiles.length===0 && aiFiles.length===0 && !aiFolder){
    return {id:subjectId,name:dirItem.name,lectures:[],ai:[],years:[],allQuestions:[],totalQuestions:0,totalLectures:0,hasAiFolder:false};
  }
  const lectures=[]; const ai=[]; let counter=1;
  for(const f of lectureFiles){
    const g=await buildLectureGroupFromFile(f,dirItem.name,subjectId,'lecture',counter);
    if(g){ counter+=g.questions.length; lectures.push(g);}
  }
  for(const f of aiFiles){
    const g=await buildLectureGroupFromFile(f,dirItem.name,subjectId,'ai',counter);
    if(g){ counter+=g.questions.length; ai.push(g);}
  }
  const all=lectures.flatMap(g=>g.questions).concat(ai.flatMap(g=>g.questions));
  const yearsMap=new Map();
  all.forEach(q=>{
    const batch=String(q.batchName||'').trim();
    if(!batch) return;
    if(!yearsMap.has(batch)) yearsMap.set(batch,[]);
    yearsMap.get(batch).push(Object.assign({},q,{sourceType:'year',originalSourceType:q.sourceType}));
  });
  const years=Array.from(yearsMap.entries()).sort((a,b)=>a[0].localeCompare(b[0],'en',{sensitivity:'base'})).map(([name,questions])=>({id:subjectId+'__year__'+slugify(name),name,type:'year',subjectName:dirItem.name,questions}));
  return {id:subjectId,name:dirItem.name,lectures,ai,years,allQuestions:all,totalQuestions:all.length,totalLectures:lectures.length+ai.length+years.length,hasAiFolder:!!aiFolder};
}
...
async function setupLogos(){
  const gallery = el('logo-gallery');
  const fallback = el('logo-fallback');
  const left = el('logo-slot-left');
  const center = el('logo-slot-center');
  const right = el('logo-slot-right');
  if(!gallery || !fallback || !left || !center || !right) return;

  const candidates = [
    { name:'logo', exts:['png','jpg','jpeg','webp','gif'] },
    { name:'logo2', exts:['png','jpg','jpeg','webp','gif'] },
    { name:'logo3', exts:['png','jpg','jpeg','webp','gif'] }
  ];

  async function resolveImage(prefix){
    for(const ext of prefix.exts){
  ...
  const logo = await resolveImage(candidates[0]);
  const logo2 = await resolveImage(candidates[1]);
  const logo3 = await resolveImage(candidates[2]);
  ...
  if(logo && logo2 && logo3){
    left.src = logo3; left.classList.remove('hidden');
    center.src = logo; center.classList.remove('hidden');
    right.src = logo2; right.classList.remove('hidden');
    return;
  }
  if(logo && logo2){
    left.src = logo2; left.classList.remove('hidden');
    right.src = logo; right.classList.remove('hidden');
    return;
  }
  if(logo){
    center.src = logo;
...
window.addEventListener('DOMContentLoaded', async ()=>{
  ...
  try{
    ...
    renderSubjects();
    renderChecklist();
    showToast('تعذر تحميل بيانات GitHub الآن. تحقّق من الاتصال أو بنية المجلدات أو تأكد من أن الفرع هو main/master/gh-pages.','error');
  }
  checkResumeExam();
});
...
window.handleSubjectOpen=handleSubjectOpen;
window.startSubjectLongPress=startSubjectLongPress;
window.cancelSubjectLongPress=cancelSubjectLongPress;
window.moveSubject=moveSubject;
window.togglePinSubject=togglePinSubject;
...
window.toggleQuestionLocation=toggleQuestionLocation;
window.toggleFavorite=toggleFavorite;
window.resetProgress=resetProgressFull;
window.goHome=goHome;
window.toggleExamSettings=toggleExamSettings;
window.hideDialog=hideDialog;
window.dialogConfirmAction=dialogConfirmAction;
window.dialogCancelAction=dialogCancelAction;
window.backToSubjects=backToSubjects;
window.confirmRemoveCurrentWrong=confirmRemoveCurrentWrong;
window.confirmBulkRemoveMasteredWrong=confirmBulkRemoveMasteredWrong;
window.openMemories=openMemories;
window.switchMemoriesTab=switchMemoriesTab;
window.renderMemories=renderMemories;
window.printMemoriesPdf=printMemoriesPdf;
window.openHistoryDeleteDialog=openHistoryDeleteDialog;
window.toggleHistoryDeleteModal=toggleHistoryDeleteModal;
window.renderHistoryDeleteList=renderHistoryDeleteList;
window.selectAllHistoryDeleteItems=selectAllHistoryDeleteItems;
window.confirmDeleteHistoryItems=confirmDeleteHistoryItems;
window.shiftMemoriesRange=shiftMemoriesRange;

window.openStatisticsPage=openStatisticsPage;
window.closeStatisticsPage=closeStatisticsPage;
window.renderStatisticsPage=renderStatisticsPage;
window.openSubjectStats=openSubjectStats;
window.closeSubjectStats=closeSubjectStats;
window.renderSubjectStats=renderSubjectStats;
window.openSubjectCategoryFromStats=openSubjectCategoryFromStats;
window.openStatsExclusionDialog=openStatsExclusionDialog;
window.closeStatsExclusionDialog=closeStatsExclusionDialog;
window.saveStatsExclusions=saveStatsExclusions;
window.applyStatsExclusions=applyStatsExclusions;
window.openResetModal=openResetModal;
window.closeResetModal=closeResetModal;
window.showResetConfirmation=showResetConfirmation;
window.goResetStep1=goResetStep1;
window.executeResetStatistics=executeResetStatistics;
window.openSubjectStatsSettings=openSubjectStatsSettings;
window.closeSubjectStatsSettings=closeSubjectStatsSettings;
window.applySubjectStatsSettings=applySubjectStatsSettings;
window.openChecklist=openChecklist;
window.renderChecklist=renderChecklist;
window.toggleChecklistSubject=toggleChecklistSubject;
window.toggleChecklistLecture=toggleChecklistLecture;
window.openChecklistSubject=openChecklistSubject;
window.closeChecklistSubject=closeChecklistSubject;
window.renderChecklistSubject=renderChecklistSubject;
